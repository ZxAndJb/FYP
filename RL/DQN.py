...
#         self.norm = torch.nn.LayerNorm([288])
#     def forward(self, x):
#         x = self.norm(x)
...
class Qnet(torch.nn.Module):
    def __init__(self, embedding_dim, weights, hidden_dim, n_layers, action_dim):
        super().__init__()
        self.word_embed = torch.nn.Embedding.from_pretrained(weights)
        self.lstm = torch.nn.LSTM(embedding_dim, hidden_dim, num_layers=n_layers, bidirectional=True, batch_first=True)
        self.picker = torch.nn.Linear(hidden_dim * 2, action_dim)

    def forward(self, text):
        embedding = self.word_embed(text)
        output, (hidden, cell) = self.lstm(embedding)
        if len(output.size()) == 2:
            elite = output[-1]
        else:
            elite = output[:, -1, :]
        output = self.picker(elite)
        return output
...
class DQN:
    ...
    def __init__(self,
        ...
        self.q_net = Qnet(100, weights, hidden_dim, 6, self.action_dim).to(device)
        self.target_q_net = Qnet(100, weights, hidden_dim, 6, self.action_dim).to(device)
    ...
    def update(self, transition_dict):
        states = torch.stack(list(transition_dict['states'])).to(self.device)
        actions = torch.tensor(transition_dict['actions']).view(-1, 1).to(
            self.device)
        rewards = torch.tensor(transition_dict['rewards'],
                               dtype=torch.float).view(-1, 1).to(self.device)
        next_states = torch.stack(list(transition_dict['next_states'])).to(self.device)
    ...
    def savemodel(self):
        torch.save(self.q_net.state_dict(), "qnet.pth")
        torch.save(self.target_q_net.state_dict(), "target_q_net.pth")


    def loadmodel(self, train, target):
        self.q_net.load_state_dict(torch.load(train))
        self.target_q_net.load_state_dict(torch.load(target))
...
if __name__ == '__main__':
    ...
    state_dim = MAX_Sen_Len * Max_SEN_Number
    ...
    agent = DQN(state_dim, hidden_dim, action_dim, lr,weight_decay, gamma, epsilon,
                target_update, device, Pre_train_embedding)
